...
from data import read_data, ReadData
import warnings
...
def main():
    ...
    if args.data_zinc is not None:
        ...
        # data = read_data(args.data_act, args.test, 10000)
        # data = read_data(args.data_act, args.test)
        pass
        # ids = data[0]
        # data = data[1:]
    ...
    if args.test:
        gen_data = ReadData(args.data_act, 10000)
        idx_test = np.loadtxt(f'{PurePosixPath(args.ae).parent}/{PurePosixPath(args.ae).name.split("-")[-1].split(".")[0]}.index')
        n_col = gen_data.n_col - idx_test.size
        print(f'Number of rows in data: {gen_data.n_row}')
    else:
        ...
        np.savetxt(f"{args.dir_save}/{PurePosixPath(args.data_act).stem.replace('_actives_onbits', '')}.index",
                   idx, fmt='%d')
    ...
    args.dims_layers_ae = [n_col] + args.dims_layers_ae
    ...
    if args.test:
        all_labels = []
        for data in gen_data.getData():
            idx = np.setdiff1d(np.arange(data.shape[1]), idx_test)
            data = data[:, idx]
            print(' Data shape:', data.shape)
            ...
            with torch.no_grad():
                i = 0
                while i * args.batch_size < x_test.shape[0]:
                    batch = torch.from_numpy(x_test[i * args.batch_size:(i + 1) * args.batch_size]).float().to(device)
                    # ===================forward=====================
                    z, output_ae = model_ae(batch)
                    labels_pred.append(model_classifier(z))
                    i += 1
                    print(f'\r{i * args.batch_size} | {x_test.shape[0]}', end = "")
                print()
            labels_pred = np.round(np.concatenate(labels_pred, axis=0)).astype(int)
            #labels_pred = np.round(np.concatenate(labels_pred.detach().cpu().numpy(), axis=0)).astype(int)
            all_labels.append(labels_pred)

        # raise ValueError(labels_pred.shape)
        # labels_pred = np.concatenate((ids.reshape((-1,1)), labels_pred), axis = 1)
        np.savetxt(f"{args.dir_save}/{PurePosixPath(args.ae).stem.replace('ae_model-', '')}-"
                       f"{PurePosixPath(args.data_act).stem.replace('_actives_onbits', '')}.lab",
                       np.concatenate(all_labels, axis=0), fmt='%d')
        # np.savetxt(f"{args.dir_save}/{PurePosixPath(args.data_act).stem.replace('_actives_onbits', '')}.lab", np.concatenate(all_labels, axis=0), fmt='%d')
    else:
        ...
        if 'pre-training_ae' in args.procedure:
            ...
            for epoch in epoch_tqdm:
                ...
                epoch_tqdm.set_description("Epoch pre-train loss: {:.5f}".format(loss_train))
        ...
        for epoch in epoch_tqdm:
            ...
            epoch_tqdm.set_description("Epoch train loss: {:.5f}".format(loss_train))
            # epoch_tqdm.set_description("Epoch train loss: {:.5f}, test loss: {:.5f}".format(loss_train, loss_test))
            # acc.append(acc_value)
            # roc_auc.append(ra)
            # m_corr.append(mc)
        writer.close()
        torch.save(model_ae.state_dict(), '{}/ae_model-{}.pth'.format(args.dir_save, PurePosixPath(args.data_act).stem.replace('_actives_onbits', '')))
        torch.save(model_classifier.state_dict(), '{}/classifier_model-{}.pth'.format(args.dir_save, PurePosixPath(args.data_act).stem.replace('_actives_onbits', '')))
